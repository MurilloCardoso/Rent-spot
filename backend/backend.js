...
app.post('/inserirDados', async (req, res) => {
  try {
    await mongoose.connect('mongodb+srv://murilloaqw:quW5gfJolEvMLDx4@rentspot.hmyt9cq.mongodb.net/', {
...
await user.save(); // Salve o modelo no banco de dados
  ...
  mongoose.connection.close();
    ...
    res.status(200).json({ message: 'Conexão estabelecida e operações realizadas com sucesso.' });
  } catch (error) {
...
console.log("awd")
  ...
  try {
    await mongoose.connect('mongodb+srv://murilloaqw:quW5gfJolEvMLDx4@rentspot.hmyt9cq.mongodb.net/sample_mflix', {
      useNewUrlParser: true,
      useUnifiedTopology: true
    });
